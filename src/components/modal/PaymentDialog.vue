<template>
  <v-app class="payment-dialog">
    <v-row justify="center">
      <v-dialog
        v-model="$store.state.payment.displayUserPayment"
        max-width="600px"
      >
        <v-card>
          <v-card-title>
            ...
            <v-row no-gutters>
              <v-col cols="4">
                <v-text-field
                  ...
                  :value="
                    $store.state.payment.userPayment.payment + ' ' + 'lei'
                  "
                ></v-text-field>
              </v-col>
            </v-row>
          ...
          <v-card-actions>
            ...
            <v-btn
              color="blue darken-1"
              text
              @click="$store.state.payment.displayUserPayment = false"
              >Close</v-btn
            >
          </v-card-actions>
        </v-card>
...
export default {
  ...
  methods: {
    ...
    getUserPaymentOnMonth() {
      ...
      this.$store.dispatch("getUserPaymentOnMonth", {
        month: month,
        year: year
      });
    },
    getUserPaymentOnPeriod() {
      if (this.dateFromPicker != null && this.dateToPicker != null) {
        ...
        this.$store.dispatch("getUserPaymentOnPeriod", {
